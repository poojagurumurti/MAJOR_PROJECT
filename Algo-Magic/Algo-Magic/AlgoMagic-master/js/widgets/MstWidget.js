...
var MST = function () {
  ...
  this.prim = function (startVertexText, mstTypeConstant) {
    ...
    if (startVertexText >= amountVertex) {
    ...
    function sortedArrayToString() {
      ...
      for (var i = 0; i < maxLength; i++) {
        ...
        ansStr +=
          "(" + thisTriple.getFirst() + "," + thisTriple.getSecond() + ")";
        if (i < maxLength - 1) {
          ansStr += ", ";
        }
      }
    ...
    for (key in internalAdjList[startVertexText]) {
      ...
      if (mstTypeConstant == MST_MAX)
        enqueuedEdge = enqueuedEdge = new ObjectTriple(
          -1 * internalEdgeList[enqueuedEdgeId]["weight"],
          key,
          enqueuedEdgeId
        );
      else
        enqueuedEdge = new ObjectTriple(
          internalEdgeList[enqueuedEdgeId]["weight"],
          key,
          enqueuedEdgeId
        );
      edgeQueued[enqueuedEdgeId] = true;
      enqueuedToString +=
        "(" + internalEdgeList[enqueuedEdgeId]["weight"] + "," + key + "), ";
      sortedArray.push(enqueuedEdge);
    }

    enqueuedToString = enqueuedToString.substring(
      0,
      enqueuedToString.length - 2
    );
    ...
    while (
      // /Object.keys(notVisited).length > 0/
      ...
      currentState["status"] =
        "Remove pair (" +
        dequeuedEdge.getFirst() +
        "," +
        otherVertex +
        ") from PQ. Check if vertex " +
        otherVertex +
      ...
      if (notVisited[otherVertex] != null) {
        ...
        currentState["status"] =
          ...
          " to T, and add " +
          enqueuedToString +
          " to the Priority Queue.<br/>The PQ is now " +
          sortedArrayToString() +
          ".";
    ...
    currentState["status"] =
      "The highlighted vertices and edges form a Minimum Spanning Tree,<br>Space complexity-O(E+V)";
  ...
  this.kruskal = function (mstTypeConstant) {
    ...
    for (key in internalEdgeList) {
      ...
      if (mstTypeConstant == MST_MAX) {
        edgeQueued[key] = true;
        enqueuedEdge = new ObjectPair(
          -1 * internalEdgeList[key]["weight"],
          parseInt(key)
        );
      } else {
        edgeQueued[key] = true;
        enqueuedEdge = new ObjectPair(
          internalEdgeList[key]["weight"],
          parseInt(key)
        );
      }
      sortedArray.push(enqueuedEdge);
    ...
    function sortedArrayToString() {
      ...
      for (var i = 0; i < maxLength; i++) {
        ...
        ansStr +=
          "(" +
          internalEdgeList[thisEdgeId]["weight"] +
          ",(" +
          internalEdgeList[thisEdgeId]["vertexA"] +
          "," +
          internalEdgeList[thisEdgeId]["vertexB"] +
          "))";
        if (i < maxLength - 1) {
          ansStr += ", ";
        }
    ...
    currentState["status"] =
      "Edges are sorted in increasing order of weight: " +
      sortedArrayToString();
    ...
    while (sortedArray.length > 0) {
      ...
      if (sortedArray.length > 1) {
        currentState["status"] =
          "The remaining edges are " + sortedArrayToString();
      } else if (sortedArray.length == 1) {
        currentState["status"] =
          "The remaining edge is " + sortedArrayToString();
      }
      ...
      if (noCycle) {
        currentState["status"] =
          ...
          ") with weight " +
          thisWeight +
          " does not form a cycle, so add it to T. The current weight of T is " +
          totalWeight +
          ".";
        currentState["lineNo"] = 5;
      } else {
        currentState["status"] =
          ...
          "," +
          vertexB +
          ") will form a cycle, so ignore it. The current weight of T remains at " +
          totalWeight +
          ".";
        currentState["lineNo"] = 6;
      }
    ...
    currentState["status"] =
      "The highlighted vertices and edges form a Minimum Spanning Tree with MST weight = " +
      totalWeight +
  ...
  function populatePseudocode(act) {
    switch (act) {
      case 0: // Prim's
        ...
        $("#code5").html(
          "&nbsp;&nbsp;&nbsp;&nbsp;T = T &cup; v, enqueue edges connected to v"
        );
