...
var Heap = function () {
  ...
  this.insert = function (vertexText, startAnimationDirectly) {
    ...
    for (i = 0; i < A.length; i++) {
      if (A[i].getFirst() == parseInt(vertexText)) {
        $("#insert-err").html(
          "Sorry, that value is already inside the heap, this visualization can only handle unique elements"
        );
        return false;
      }
    }
    ...
    currentState["status"] =
      "Insert " + vertexText + " at the back of compact array A";
    if (!startAnimationDirectly) {
      //buildv1
    ...
    while (i > 1 && A[parent(i)].getFirst() < A[i].getFirst()) {
      ...
      currentState["vl"][A[parent(i)].getSecond()]["state"] =
        VERTEX_HIGHLIGHTED;
      currentState["status"] =
        A[i].getFirst() +
        " and " +
    ...
    currentState["status"] =
      "Insertion of " +
      vertexText +
      " has been done successfully ,<br> Space complexity-O(Logn)";
    if (!startAnimationDirectly) {
      //buildv1
      currentState["lineNo"] = 3;
    } else {
  ...
  this.extractMax = function (startAnimationDirectly) {
    if (A.length == 2) {
      $("#extractmax-err").html(
        "Sorry, the Max Heap contains only one item. This is the maximum element. We cannot delete any more items."
      );
      return false;
    }
  ...
  this.heapSort = function () {
    ...
    for (
      var i = 0;
      i < len - 1;
      i++ // except the last item (minor bug if not stopped here)
    )
  ...
  this.buildV1 = function (arr) {
    if (arr.length > 31) {
      $("#buildv1-err").html(
        "Sorry, you cannot build a Max Heap with more than 31 elements in this visualization."
      );
      return false;
    }
    ...
    currentState = createState(A);
    currentState["status"] =
      "The Max Heap has been successfully built from input array: " +
      arr +
  ...
  this.buildV2 = function (arr) {
    ...
    for (i = 0; i < arr.length; i++) initValue[i + 1] = parseInt(arr[i]);
    ...
    var currentState = createState(A);
    currentState["status"] =
      "First, copy the entire content of the input array into a Complete Binary Tree structure";
    currentState["lineNo"] = 1;
    stateList.push(currentState);

    for (i = Math.floor(arr.length / 2); i >= 1; i--) {
      // check heap property one by one
      currentState = createState(A);
      currentState["vl"][A[i].getSecond()]["state"] = VERTEX_HIGHLIGHTED;
      currentState["status"] =
        "Calling ShiftDown(" +
        i +
        ") to fix Max Heap property of subtree rooted at " +
        A[i].getFirst() +
        ", if necessary";
      currentState["lineNo"] = [2, 3];
      stateList.push(currentState);
    ...
    switch (act) {
      case 0: // Insert
        ...
        document.getElementById("code4").innerHTML =
          "&nbsp&nbspswap A[i] and A[parent(i)]";
      ...
      case 1: // ExtractMax
        ...
        document.getElementById("code5").innerHTML =
          "&nbsp&nbspif A[i] < than the larger of its children";
        document.getElementById("code6").innerHTML =
          "&nbsp&nbsp&nbsp&nbspswap A[i] with that child";
        document.getElementById("code7").innerHTML = "";
        break;
      case 2: // HeapSort
  ...
  function createState(internalHeapObject) {
    ...
    for (var i = 2; i < internalHeapObject.length; i++) {
      ...
      state["el"][edgeId]["vertexA"] =
        internalHeapObject[parent(i)].getSecond();
