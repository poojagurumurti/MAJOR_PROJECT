...
var SSSP = function () {
  ...
  this.bfs = function (sourceVertex) {
    ...
    if (amountVertex == 0) {
      // no graph
      $("#bfs-err").html(
        "There is no graph to run this on. Please select a sample graph first."
      );
      return false;
    }

    if (sourceVertex >= amountVertex) {
    ...
    for (var j = 0; j < amountEdge; j++)
      if (internalEdgeList[j]["weight"] > 1) {
        $("#bfs-err").html(
          "At least one edge of this graph has weight > 1. We cannot run BFS to get the SSSP information of this graph."
        );
        return false;
      }
    ...
    for (key in internalAdjList) internalAdjList[key]["state"] = VERTEX_DEFAULT;
    ...
    for (key in internalEdgeList) delete edgeTraversed[key];
    ...
    while (q.length > 0) {
      ...
      currentState["status"] =
        "The queue is now {" +
        q +
        "}<br>Exploring neighbors of vertex u = " +
        q[0];
        ...
        if (u == vertexA) {
          ...
          if (d[vertexA] + weightAB < d[vertexB]) {
            ...
            thisStatus =
              ...
              p[vertexB];
            q.push(vertexB);
          } else thisStatus = thisStatus + "<br>No change";
          ...
          for (var k = 0; k < amountVertex; k++)
            if (p[k] != -1)
              for (var l = 0; l < amountEdge; l++)
                if (
                  internalEdgeList[l]["vertexA"] == p[k] &&
                  internalEdgeList[l]["vertexB"] == k
                )
    ...
    for (key in internalAdjList) delete vertexHighlighted[key];
    for (key in internalAdjList) delete vertexTraversed[key];
    for (key in internalEdgeList) delete edgeHighlighted[key];
    edgeTraversed = {};
    currentState = createState(
      internalAdjList,
      internalEdgeList,
    ...
    currentState["status"] =
      "BFS processes O(V + E) = " +
      EdgeProcessed +
      " edges.<br>The BFS/SSSP spanning tree from source = " +
      sourceVertex +
      " is shown on the right side. Space complexity-O(|V|)";
    stateList.push(currentState);
  ...
  this.bellmanford = function (sourceVertex) {
    ...
    for (var i = 1; i < amountVertex; i++) {
      ...
      for (var j = 0; j < amountEdge; j++) {
        ...
        for (var k = 0; k < 2 * amountVertex; k++) delete vertexHighlighted[k];
        for (var k = amountEdge; k < 2 * amountEdge; k++)
          internalEdgeList[k]["state"] = OBJ_HIDDEN;
        for (var k = amountEdge; k < 2 * amountEdge; k++)
    ...
    for (var k = 0; k < 2 * amountVertex; k++) delete vertexHighlighted[k];
    for (var k = 0; k < 2 * amountEdge; k++) delete edgeHighlighted[k];
    currentState = createState(
      internalAdjList,
      internalEdgeList,
      vertexHighlighted,
    ...
    currentState["status"] =
      "Bellman Ford's processes O(VE) = " +
      (amountVertex - 1) +
      "*" +
      amountEdge +
      " = " +
      EdgeProcessed +
      "  edges.<br>The SSSP spanning tree from source = " +
      sourceVertex +
      " is shown on the right side. Space complexity-O(V)";
    stateList.push(currentState);
  ...
  this.dijkstra = function (sourceVertex, versiontype) {
    ...
    if (versiontype == 1) {
      ...
      for (var i = 0; i < amountVertex; i++)
        ...
        else pq.push(new ObjectPair(1000, i));
    } // modified
    else pq.push(new ObjectPair(0, sourceVertex)); // only push one

    var EdgeProcessed = 0;

    while (pq.length > 0) {
      pq.sort(ObjectPair.compare); // sort by distance, then by vertex number, lousy O(n log n) PQ update
      if (versiontype == 2 && EdgeProcessed > 50) {
        // to prevent infinite loop in Modified Dijkstra on negative cycle
        currentState = createState(
          internalAdjList,
          internalEdgeList,
          vertexHighlighted,
        ...
        currentState["status"] =
          "Modified Dijkstra's algorithm is stopped prematurely<br>in order to prevent infinite loop";
        stateList.push(currentState);
        break;
      }
      ...
      currentState["status"] +=
        pq[0].getFirst() + "," + pq[0].getSecond() + ")";
      if (pq.length > 1)
        currentState["status"] +=
          ", (" + pq[1].getFirst() + "," + pq[1].getSecond() + ")";
      if (pq.length > 2)
        currentState["status"] +=
          ", (" + pq[2].getFirst() + "," + pq[2].getSecond() + ")";
      if (pq.length > 3) currentState["status"] += ", ...";
      ...
      if (versiontype == 2 && dist > d[u]) {
        currentState["status"] +=
          ...
          "," +
          u +
          ") is an old information and skipped";
      ...
      for (var j = 0; j < amountEdge; j++) {
        ...
        if (u == vertexA) {
          ...
          if (d[vertexA] + weightAB < d[vertexB]) {
            ...
            if (versiontype == 1)
              for (
                var k = 0;
                k < pq.length;
                k++ // lousy O(n) PQ update, but it works for this animation (only for version 1)
              )
            ...
            if (versiontype == 2 || canRelaxThis)
              // for standard dijkstra
          ...
          currentState["status"] = thisStatus;
          if (versiontype == 1) currentState["lineNo"] = [3, 4];
          else currentState["lineNo"] = [4, 5];
          stateList.push(currentState);
        }
      }
    }

    if (versiontype == 1 || (versiontype == 2 && EdgeProcessed <= 50)) {
      // to prevent infinite loop in Modified Dijkstra on negative cycle
      for (key in internalAdjList) delete vertexHighlighted[key];
      for (key in internalAdjList) delete vertexTraversed[key];
      for (key in internalEdgeList) delete edgeHighlighted[key];
      ...
      currentState["status"] =
        "Dijkstra's processes O((V + E) * log V) = " +
        EdgeProcessed +
        " edges.<br>The SSSP spanning tree from source = " +
        sourceVertex +
        " is shown on the right side. Space complexity-O(V2)";
      stateList.push(currentState);
    }

    console.log(stateList);

    if (versiontype == 1) populatePseudocode(2);
    else populatePseudocode(3);
  ...
  this.examples = function (ssspExampleConstant) {
    if (internalAdjList != null) {
      for (key in internalAdjList) delete internalAdjList[key];
    }
    if (internalEdgeList != null) {
      for (key in internalEdgeList) delete internalEdgeList[key];
    }
  ...
  function createState(
    internalAdjListObject,
    internalEdgeListObject,
    vertexHighlighted,
    edgeHighlighted,
    vertexTraversed,
    edgeTraversed
